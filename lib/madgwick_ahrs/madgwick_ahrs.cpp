...
void MadgwickAHRS::set_filter_params(float _beta, float _T)
{
	beta = _beta;
	T = _T;
}

void MadgwickAHRS::set_init_orientation(const double* _accel, const double* _mag)
{
	double euler_init[3];
	accel_to_roll_pitch(_accel, euler_init[0], euler_init[1]);
	mag_to_yaw(_mag, euler_init[0], euler_init[1], euler_init[2]);

	memcpy(euler, euler_init, sizeof(double) * 3U);
	euler_to_quat(euler_init, quat);
...
float MadgwickAHRS::inv_sqrt(float x) {
	float halfx = 0.5f * x;
	float y = x;
	long i = *(long*)&y;
	i = 0x5f3759df - (i >> 1);
